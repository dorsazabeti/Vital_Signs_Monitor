...
مسیر شبکه بدون وابستگی به LwIP تکمیل شده است. برد با IP ثابت از طریق Ethernet هشدار را به‌صورت UDP/JSON به رایانه می‌فرستد. برنامهٔ Python رایانه پیام را اعتبارسنجی می‌کند، در ترمینال نشان می‌دهد، در فایل ثبت می‌کند و روی macOS اعلان دسکتاپ می‌سازد. در صورت نیاز همان پیام می‌تواند به یک HTTP webhook نیز Forward شود.
...
همهٔ مقادیر شبکه فقط در `Core/Inc/network_alert_config.h` قرار دارند. حدهای علائم حیاتی در `Core/Inc/vitals_thresholds.h` مشترک‌اند تا رنگ UI و پیام شبکه همیشه تصمیم یکسانی بگیرند. برای اتصال از طریق مودم یا سوییچ، IP برد، IP گیرنده، netmask و gateway را متناسب با شبکه عوض کنید. IP انتخاب‌شده برای برد نباید توسط وسیلهٔ دیگری استفاده شود.
...
یا همهٔ تست‌های Python و C را یک‌جا اجرا کنید:
...
- کد شبکه از `freertos.c` راه‌اندازی و سرویس می‌شود و رابط عمومی آن مستقل از UI باقی مانده است.
