...
| Build کامل ARM | نیازمند ابزار ARM | روی سیستم فعلی `cmake` و `arm-none-eabi-gcc` نصب نیست |
...
- Build نهایی ARM: روی این سیستم اجرا نشد، چون CMake و GNU Arm Toolchain نصب نیستند.

Build Firmware پس از نصب ابزارها:
...
1. نصب/انتخاب GNU Arm Toolchain و اجرای Build نهایی؛ بررسی تولید ELF/HEX/BIN و اندازه Flash.
2. Flash کردن BIN یا ELF روی برد.
3. مشاهده داشبورد و اجرای پنج سناریو روی UART؛ ثبت عکس یا ویدئو.
4. توقف شبیه‌ساز و تأیید ظاهر شدن `NO DATA` حداکثر بعد از دو ثانیه.
5. تنظیم Ethernet مک روی `192.168.7.1/24` و تأیید Link و `ping 192.168.7.2`.
6. اجرای `Low_SpO2`، مشاهده `triggered` و ثبت خط در `alerts.jsonl`؛ سپس Normal و مشاهده `recovered`.
7. ثبت Commit hash نسخه Demo و نتایج بالا در گزارش نهایی.
...
- [ ] Build کامل با `arm-none-eabi-gcc`
- [ ] ELF/HEX/BIN و اندازه مجاز Flash
...
Commit پایه پس از دریافت کار هم‌تیمی:

```text
8fe6046 Temperature label
```

هنگام شروع یکپارچه‌سازی، `main` و `origin/main` همگام بودند. تغییرات نهایی این سند و کد در Working Tree قرار دارند و مطابق درخواست درسا هیچ Commit خودکاری ایجاد نشده است.
